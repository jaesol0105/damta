...
class FixedColors extends ThemeExtension<FixedColors> {
  const FixedColors({required this.brandColor, required this.blue});
  final Color? brandColor;
  final Color? blue;

  static const FixedColors constant = FixedColors(
    brandColor: Color(0xFFA7D7FE), // 앱 브랜드 컬러 (급식/학사일정에서 쓴거)
    blue: Color(0xFFD5ECFF), // 앱 브랜드 컬러 (혜린님이 테마에 정의 해두신거)
  );

  /// Flutter는 Theme.of(context)로 가져온 객체를 불변(immutable) 으로 간주함.
  /// 복사본을 기반으로 새로운 테마 상태를 만들기 위해 copyWith()가 필요.
  @override
  FixedColors copyWith({Color? brandColor, Color? blue}) =>
      FixedColors(brandColor: brandColor ?? this.brandColor, blue: blue ?? this.blue);
