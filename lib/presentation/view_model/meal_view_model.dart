...
abstract class MealState with _$MealState {
  ...
  List<MealEntity> get mealsForSelectedDate {
    return allMeals.where((meal) {
      ...
      return meal.date.year == d.year && meal.date.month == d.month && meal.date.day == d.day;
    }).toList();
  }
...
class MealViewModel extends _$MealViewModel {
  @override
  Future<MealState> build({required String officeCode, required String schoolCode}) async {
  ...
  Future<MealState> _fetchMeals(String officeCode, String schoolCode) async {
    ...
    if (!hasTodayMeal && meals.isNotEmpty) {
      final future = meals.firstWhere((m) => !m.date.isBefore(selected), orElse: () => meals.first);
