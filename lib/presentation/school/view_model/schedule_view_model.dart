...
abstract class ScheduleState with _$ScheduleState {
  ...
  Map<DateTime, List<ScheduleEntity>>
  get groupedSchedulesByDateExcludingWeekends {
...
class ScheduleViewModel extends _$ScheduleViewModel {
  ...
  /// 학사일정 가져오기
  Future<ScheduleState> _fetchSchedules(
    String officeCode,
    String schoolCode,
  ) async {
  ...
  Future<void> reload() async {
    ...
    state = await AsyncValue.guard(
      () => _fetchSchedules(officeCode, schoolCode),
    );
  }
  ...
  void selectMonth(int year, int month) {
    ...
    state = AsyncData(
      current.copyWith(selectedYear: year, selectedMonth: month),
    );
  }
}
