...
class PostListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final commentCount = comments.where((c) => c.pId == post.pId).length; // 댓글 개수 계산
            ...
            IntrinsicHeight(
              child: Row(
                children: [
                  ...
                  Text(commentCount.toString(), style: TextStyle(color: vrc(context).detailText)),
                  ...
                  const SizedBox(width: 10),
                  Text(timeAgo(post.pCreatedAt), style: TextStyle(color: vrc(context).detailText)),
                  const SizedBox(width: 10),
                  Container(width: 2, height: 12, color: vrc(context).border),
                  const SizedBox(width: 10),
                  Text(
